/**
 * Provision User — Reneesha D. Parrish (Admin)
 *
 ...
 *
 * Prerequisites:
 *   - Account already exists in Firebase Auth (reneesha@parrishhealthsystems.org)
 *   - service-account.json in project root (or GOOGLE_APPLICATION_CREDENTIALS set)
 *
 * Usage:
 *   npx tsx scripts/seed/provisionReneesha.ts
 */
...
// ── User to provision ─────────────────────────────
const USER = {
  email: 'reneesha@parrishhealthsystems.org',
  displayName: 'Reneesha D. Parrish, RN, BSN',
  role: 'admin' as const,
...
async function provision() {
  ...
  console.log('\n✅ Done! Reneesha is ready to log in.');
  console.log(`   Email: ${USER.email}`);
  console.log(`   Role:  ${USER.role}`);
  console.log(`   UID:   ${uid}`);
  console.log('\n   You may need to sign out and back in if already logged in');
